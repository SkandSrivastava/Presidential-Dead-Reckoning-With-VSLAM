function [Acc,Gyr,AccX,GyrX]=ReadLogFile(filename,ver,idx_fig)
...
%   ver:         Controla los graficos y datos de salida que nos interesan para la prแctica. Opciones: 'smartphone', 'Xsens', 'full'(default)
...
while (~eof)
    ...
    while (~linea_con_medida && ~eof)
        ...
        if ( idx_linea<=numlineas) % hay lineas por procesar
            ...
            % Orientation data:   	'AHRS;AppTimestamp(s);SensorTimestamp(s);PitchX(ยบ);RollY(ยบ);YawZ(ยบ);RotVecX();RotVecY();RotVecZ();Accuracy(int)'
            ...
            % IMU XSens data:     	'IMUX;AppTimestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(ยบ);Pitch(ยบ);Yaw(ยบ);Pressure(mbar);Temp(ยบC)'
            % IMU LPMS-B data:    	'IMUL;AppTimestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(ยบ);Pitch(ยบ);Yaw(ยบ);Pressure(mbar);Temp(ยบC)'
...
if ~isempty(Gnss) && strcmp(ver,'full')
    figure(idx_fig); idx_fig=idx_fig+1;
    subplot(4,2,1);
    plot(Gnss(:,1),Gnss(:,3),'r-');ylabel('Lat(บ)');
    title(['GNSS data - Freq: ',num2str(size(Gnss,1)/Gnss(end,1)),' Hz']);
    subplot(4,2,2);
    plot(Gnss(:,1),Gnss(:,4),'r-'); ylabel('Long(บ)');
    ...
    subplot(4,2,5);
    plot(Gnss(:,1),Gnss(:,6),'r-'); ylabel('Bearing(บ)');
    subplot(4,2,6);
